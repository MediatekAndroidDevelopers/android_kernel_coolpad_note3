...
static int ltr559_proximity_fast_calibrate(void)
{
    int i = 0;
    u16 prox_sum = 0, prox_mean = 0;
    u16 ps_threshold_high, ps_threshold_low;
...
//yangxiaogen@yulong.com modify parameter for improve the psensor accuracy 20151223 begin
    ps_threshold_high = (u16)(prox_mean + 55);
        ps_threshold_low  = (u16)(prox_mean + 35);
//yangxiaogen@yulong.com modify parameter for improve the psensor accuracy 20151223 end
//  else
//  {
//      printk(KERN_ERR "Clibrate failed!\n");
//      atomic_set(&dev->ps_cal_result, 30);
...
//yangxiaogen@yulong.com del for proximity calibration begin at 20151211
   // mdelay(WAKEUP_DELAY);
//yangxiaogen@yulong.com del for proximity calibration end at 20151211
    /* =============== 
     * ** IMPORTANT **
     * ===============
...
//yangxiaogen@yulong.com add for proximity calibration begin at 20151211
    mdelay(WAKEUP_DELAY);
    ltr559_proximity_fast_calibrate();
//yangxiaogen@yulong.com add for proximity calibration end at 20151211
...
//begin baibo@yulong.com 20151023 for resolve als value exception
static int ltr559_als_read(int gainrange)
{
...
//end baibo@yulong.com 20151023 for resolve als value exception
...
/*chenbing3@yulong.com modify for no interrupt in init 20150810 end*/
    ...
    /*chenbing3@yulong.com the first ps detection always close in init 20151125 begin*/
    res = ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 12);
    if(res<0)
    {
        APS_LOG("ltr559_ps_enable() error ltr559_ps_n_pulses\n");
        return res;
    ...
    res = ltr559_i2c_write_reg(LTR559_PS_LED, 0x9F);
    if(res<0)
    {
         APS_LOG("ltr559_ps_enable() error LTR559_PS_LED...\n");
         return res;
    }
    /*chenbing3@yulong.com the first ps detection always close in init 20151123 end*/
...
/*yangxiaogen@yulong.com del for proximity calibration in 20151210 begin*/
        //ltr559_proximity_fast_calibrate();  //wangyufei@yulong.com add for fast_calibration at 20141124
/*yangxiaogen@yulong.com del for proximity calibration in 20151210 end*/
                    set_bit(CMC_BIT_PS, &obj->enable);
                }
                else
                {
...
static int als_enable_nodata(int en)
{
	...
	mutex_lock(&LTR559_mutex);
		...
		if(en)
		{
			...
			if (res < 0) {
                APS_ERR("als_enable_nodata enable is failed!!\n");
                return -1;
			}
        }
		else
        {
			...
			if (res < 0) {
                APS_ERR("als_enable_nodata disable is failed!!\n");
                return -1;
			}
        }
        return 0;
...
/* begin baibo@yulong.com 20150720 for decice info */
#ifdef SENSOR_DEVICE_INFO
  sprintf(tembuff,"ALSPS: LTR559 LRTEON\n");
