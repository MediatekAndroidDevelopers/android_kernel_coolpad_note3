...
struct platform_device *epl259x_alspsPltFmDev;
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    dynk_low_offset =120;               //ftm modify ps  60
    dynk_high_offset =370;			   //ftm modify ps 180
#endif /*PS_DYN_K*/
...
if(g_ps_cali_flag && g_ps_base_value >0 && g_ps_base_value < 450){
	  if( (dynk_min_ps_raw_data < epl_sensor.ps.data.data)
			 ...
			 if(enable_ps == true)
            {
				...
				dynk_thd_low =  epl_sensor.ps.data.data + 30 ;// dynk_min_ps_raw_data + dynk_low_offset;
    		    dynk_thd_high = g_ps_base_value + 80;//dynk_min_ps_raw_data + dynk_high_offset;
...
static void epl_sensor_eint_work(struct work_struct *work)
{
    ...
    if(epl_sensor.als.interrupt_flag == EPL_INT_TRIGGER)
    {
	  ...
	  //  printk("geroge -------3---epl_sensor_obj->irq-------------->%d\n",epl_sensor_obj->irq);
	enable_irq(obj->irq);
...
*/
	   ...
	   epl259x_alspsPltFmDev = get_alsps_platformdev();
	/* gpio setting */
		pinctrl = devm_pinctrl_get(&epl259x_alspsPltFmDev->dev);
		if (IS_ERR(pinctrl)) {
			ret = PTR_ERR(pinctrl);
...
#if 0
static void epl259x_ps_cali_set_threshold(void){
...
//  printk("%s:geroge 22  hw->i2c_addr=%d\n", __func__,hw->i2c_addr);
	client->addr=0x92>>1;//obj->hw->i2c_addr[0];//zhiwei add for i2c addr from dts
