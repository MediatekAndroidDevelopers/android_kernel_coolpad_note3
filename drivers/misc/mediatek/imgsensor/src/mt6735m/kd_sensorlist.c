...
#if 1	//add alex.shi for peripherals start
...
static char camera_main_name[32]="Unknown";
static char camera_sub_name[32]="Unknown";

static void zyt_sensor_set_sensor(char *sensor_name, UINT32 i)
{
	if (NULL == sensor_name)
	{
		printk("<<<< alex.shi %s(): sensor_name null!\n", __func__);
		return;
	}

	printk("<<<< alex.shi %s(): sensor_name is %s, g_invokeSocketIdx[i]=%d\n", __func__, sensor_name, g_invokeSocketIdx[i]);

	if (DUAL_CAMERA_MAIN_SENSOR == g_invokeSocketIdx[i])
	{
		strcpy((char*)camera_main_name,(char*)sensor_name);
	}
	else if (g_invokeSocketIdx[i] == DUAL_CAMERA_SUB_SENSOR)
	{
		strcpy((char*)camera_sub_name,(char*)sensor_name);
	}
	else
	{
		printk("<<<< alex.shi %s(): g_invokeSocketIdx[%d] is invalid!\n", __func__, i);
	}
}

void Sensor_GetSensor_Index_info(int sensorindex, char * sensor_name_index,int *true_false)
{
	if(main_sensor == sensorindex)
	{
		strcpy(sensor_name_index, camera_main_name);
		*true_false=1;
	}
	else if (sub_sensor == sensorindex)
	{
		strcpy(sensor_name_index, camera_sub_name);
		*true_false=1;
	}
	else
		*true_false=0;
...
EXPORT_SYMBOL_GPL(Sensor_GetSensor_Index_info);
#endif
/*******************************************************************************
			...
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
						 ...
						 g_invokeSocketIdx[i], g_invokeSensorNameStr[i]);
					#if 1	// alex.shi add for hardware info *#*#9988#*#*
		     			zyt_sensor_set_sensor(g_invokeSensorNameStr[i], i);
	     			#endif
					err = ERROR_NONE;
				}
				if (ERROR_NONE != err) {
...
static long CAMERA_HW_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case KDIMGSENSORIOC_X_CUST_CLOSE_SENSOR:
...
//zhuyifeng
UINT32 gSearchSensor;

int kdSearchSensor(u32 status)
{
	PK_DBG("[kdSearchSensor] search sensor status = %d \n", status);

	spin_lock(&kdsensor_drv_lock);
	gSearchSensor = status;
	spin_unlock(&kdsensor_drv_lock);
...
/*******************************************************************************
* to close sensor which have been searched. 
*
********************************************************************************/
static inline int custom_CAMERA_HW_Close(void)
{
	kdModulePowerOn((CAMERA_DUAL_CAMERA_SENSOR_ENUM*)g_invokeSocketIdx, g_invokeSensorNameStr, false, CAMERA_HW_DRVNAME1);
			    ...
			    unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	//zhuyifeng
	case KDIMGSENSORIOC_X_SEARCH_SENSOR:
		i4RetValue = kdSearchSensor(*pIdx);
		break;	
	case KDIMGSENSORIOC_X_CUST_CLOSE_SENSOR:
    	i4RetValue = custom_CAMERA_HW_Close();
		break;
	default:
