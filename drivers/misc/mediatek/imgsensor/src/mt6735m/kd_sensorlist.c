...
/****************************************************************
*****write camera info to /dev/camera_n by weiwenping Start 20160122****
*****************************************************************/
static char sCameraInfoBuf[50] = {0};

int get_camera_info(char* buf)
{
    UINT32 len = 0;

    if (NULL == buf)
    {
        printk("get camera info fail\n");
        return -1;
    }
    ...
    if (0 > len || 50 < len)
    {
        printk("camera info buf error\n");
        return -1;
    }

    memset(sCameraInfoBuf, 0, sizeof(sCameraInfoBuf));

    strncpy(sCameraInfoBuf, buf, len);
    printk("camera info buf is : %s\n", sCameraInfoBuf);

    return 0;
}

#if 1
static int YL_SetCameraDeviceInfo(UINT32 CamCount)
{
    loff_t pos;
    ssize_t err;
    mm_segment_t old_fs;
    ...
    sprintf(filename, "/dev/camerainfo/camera%d", CamCount);

    pfile = filp_open(filename, O_WRONLY|O_CREAT|O_TRUNC, 0777);
    ...
    err = vfs_write(pfile, sCameraInfoBuf, sizeof(sCameraInfoBuf), &pos);
    if (err < 0)
    {
        printk("write camera info to file failed!\n");
    }
...
#endif
/*******************************************************
*****write camera info to /dev/camera_n by weiwenping End****
********************************************************/
			...
			if (DUAL_CAMERA_NONE_SENSOR != g_invokeSocketIdx[i]) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					snprintf(mtk_ccm_name, sizeof(mtk_ccm_name),
						 ...
						 g_invokeSocketIdx[i], g_invokeSensorNameStr[i]);
					YL_SetCameraDeviceInfo(sCamCount);
                    sCamCount++;
					#ifdef CONFIG_YL_CAM_MODULE_NAME
