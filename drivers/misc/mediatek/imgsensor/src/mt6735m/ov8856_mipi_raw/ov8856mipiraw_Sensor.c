...
#define RG_Ratio_Typical   0x135
#define BG_Ratio_Typical   0x160
struct otp_struct otp_ptr;
//static bool otp_read_flag = FALSE;
static int ov8856_ofilm_module_id = 0;
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8856MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			sensor_id_1 = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (sensor_id_1 == 0x885a) {//imgsensor_info.sensor_id) {
				LOG_INF("sensor detect success,i2c write id: 0x%x, sensor_id_1: 0x%x\n", imgsensor.i2c_write_id,sensor_id_1);	
				//return ERROR_NONE;
				break;
				}
	...
	do{
		...
		*sensor_id = ((read_cmos_sensor(0x7001) << 8) | read_cmos_sensor(0x7002));
        ...
        LOG_INF("sensor detect success,i2c write id: 0x%x, *sensor_id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
	if ((sensor_id_1 != 0x885a) || (*sensor_id!=0x8856) ){//imgsensor_info.sensor_id )|| (*sensor_id!=0x8856) ){
		// if Sensor ID is not correct, Must set *sensor_id to 0xFFFFFFFF 
		LOG_INF("sensor detect fail! \n");
		#ifdef CONFIG_YL_CAM_MODULE_NAME
		sprintf(cam_module_name, "ov8856 unknow");
		#endif
	...
	if(0x07 == ov8856_ofilm_module_id)
	{
		//*sensor_id = imgsensor_info.sensor_id;
		sprintf(factory_module_id,"sub_camera:8M-Camera ov8856-ofilm\n");
		#ifdef CONFIG_YL_CAM_MODULE_NAME
		sprintf(cam_module_name, "1 ov8856 OFilm");
		#endif
	}
	//else
	//{
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		...
		do {
			...
			if (sensor_id == 0x885a)//imgsensor_info.sensor_id) 
			{				
	...
	if (sensor_id != 0x885a)//(imgsensor_info.sensor_id != sensor_id)
		return ERROR_SENSOR_CONNECT_FAIL;
...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	ov8856_ofilm_module_id = (*otp_ptr).module_integrator_id;
	...
	LOG_INF("(* otp_ptr).checksum=%x, checksum2=%x\n",(* otp_ptr).checksum, checksum2);
...
int apply_otp(struct otp_struct *otp_ptr)
{
	...
	//LOG_INF("Line = %d, read = 0x%x, write = 0x%x\n", __LINE__, read_cmos_sensor(0x5019),R_gain>>8);
	write_cmos_sensor(0x501a, R_gain & 0x00ff);
	//LOG_INF("Line = %d, read = 0x%x, write = 0x%x\n", __LINE__, read_cmos_sensor(0x501a),R_gain & 0x00ff);
	}
	if (G_gain>0x400) {
	write_cmos_sensor(0x501b, G_gain>>8);
	//LOG_INF("Line = %d, read = 0x%x, write = 0x%x\n", __LINE__, read_cmos_sensor(0x501b),G_gain>>8);
	write_cmos_sensor(0x501c, G_gain & 0x00ff);
	//LOG_INF("Line = %d, read = 0x%x, write = 0x%x\n", __LINE__, read_cmos_sensor(0x501c),G_gain & 0x00ff);
	}
	if (B_gain>0x400) {
	write_cmos_sensor(0x501d, B_gain>>8);
	//LOG_INF("Line = %d, read = 0x%x, write = 0x%x\n", __LINE__, read_cmos_sensor(0x501d),B_gain>>8);
	write_cmos_sensor(0x501e, B_gain & 0x00ff);
	//LOG_INF("Line = %d, read = 0x%x, write = 0x%x\n", __LINE__, read_cmos_sensor(0x501e),B_gain & 0x00ff);
	}
	}
	...
	//LOG_INF("Line = %d, read = 0x%x, write = 0x%x\n", __LINE__, read_cmos_sensor(0x5000),temp);
	for(i=0;i<240;i++) {
	write_cmos_sensor(0x5900 + i, (*otp_ptr).lenc[i]);
	//LOG_INF("Line = %d, read = 0x%x, write = 0x%x\n", __LINE__, read_cmos_sensor(0x5900 + i), (*otp_ptr).lenc[i]);
	//LOG_INF("(* otp_ptr).lenc[%d]=%x\n",i,(*otp_ptr).lenc[i]);
	}
	}
	LOG_INF("exit apply otp\n");
	return (*otp_ptr).flag;
}
