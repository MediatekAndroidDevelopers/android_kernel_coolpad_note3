...
extern int set_camera_module_name(char* buf);
static char cam_module_name[64] = {0};
#endif
...
/****add by weiwenping****imx219 read lsc and awb****start****/
...
static int imx219_zhengqiao_module_id = 0;
u8 IMX219OTPData[20];

static int IMX219_RG_Ratio_Typical = 0x14b;
static int IMX219_BG_Ratio_Typical = 0x121;
static int IMX219_GG_Ratio_Typical = 0x203;

static int IMX219_readPage(int page, unsigned char *out_buf)
{
    int cnt;
    int flag;
    int temp;
    ...
    flag = (unsigned char)read_cmos_sensor(OTP_DRV_FLAG_ADDR);
    if(page == 0 || page == 5)
    {
        if(flag == 0)
        {
        ...
        imx219_zhengqiao_module_id = read_cmos_sensor(0x3205);
        printk("----wwp----module_integrator_id : %d\n",imx219_zhengqiao_module_id);

        temp = read_cmos_sensor(0x320A);
        printk("----wwp----lens_id : %d\n",temp);
    }

    //read OTP buffer to PC buffer
    for(myreg = OTP_DRV_START_ADDR; myreg <= OTP_DRV_END_ADDR; myreg++)
    {
        out_buf[myreg-0x3204] = (unsigned char)read_cmos_sensor(myreg);
    }
...
/*static void IMX219_ApplyLSC(unsigned char *LSCData)
{
    int i;

    for ( i = 0; i < OTP_DRV_LSC_SIZE; i++ )
    {
        write_cmos_sensor(0xd200+i, (int)(0xff&LSCData[i]));
    }

    for ( i = 0; i < OTP_DRV_LSC_SIZE; i++ )
    {
        write_cmos_sensor(0xd248+i, (int)(0xff&LSCData[i+70]));
    }

    for ( i = 0; i < OTP_DRV_LSC_SIZE; i++ )
    {
        write_cmos_sensor(0xd290+i, (int)(0xff&LSCData[i+140]));
    }

    for ( i = 0; i < OTP_DRV_LSC_SIZE; i++ )
    {
        write_cmos_sensor(0xd2d8+i, (int)(0xff&LSCData[i+210]));
    }

    //enable it
    write_cmos_sensor(0x0190, 0x01);
    write_cmos_sensor(0x0192, 0x00);//0x00
...
static unsigned char tempData[512]={0};
static void IMX219_ReadLSC(void)
{
    int i ,rc, group;
    unsigned int sum = 0;
    static char read_once = 0;
    ...
    if (0 == read_once)
    {
        for(group = 2; group > 0; group--)
        {
            for(i = 0; i < 5; i++)
            {
                rc = IMX219_readPage((5*(group -1) + i), tempData+i*64);
                if(rc == 0||rc == -1)
                {
                    printk("----wwp----otp info is not in group %d\n", group);
                    break;
                }
            }
        }
        ...
        if( sum != tempData[296] )
        {
            printk("otp info check error  sum = 0x%x,tmp = 0x%x !\n", sum, tempData[296]);
            return ;
        }
        #endif
        printk("otp info check sum = 0x%x,tmp = 0x%x !\n", sum, tempData[296]);
        read_once = 1;
    }

    //update LSC
    //IMX219_ApplyLSC(tempData+16);
}

//read awb at 0x3204 & 0x3214
int32_t IMX219_ReadAWB(void)
{
    int i = 0;
    int cnt = 0;
    int flag = 0;
    uint32_t uAwbFlagAddr = 0;

    if(NULL == IMX219OTPData)
    {
        return -1;
    }
    ...
    //set page 10
    write_cmos_sensor(0x3202, 10);

    for (i = 1; i > -1; i--)
    {
      uAwbFlagAddr = OTP_DRV_FLAG_ADDR + i*16;
      flag = (unsigned char)read_cmos_sensor(uAwbFlagAddr);
      if(1 == flag)
      {
          //read OTP buffer to PC buffer
          for(cnt = 0; cnt < 6; cnt++)
          {
              IMX219OTPData[cnt]=(unsigned char)read_cmos_sensor(uAwbFlagAddr + cnt + 1);
          }

          IMX219OTPData[cnt] = (IMX219_RG_Ratio_Typical&0xff00)>>8;
          IMX219OTPData[cnt+1] = (IMX219_RG_Ratio_Typical&0xff);
          IMX219OTPData[cnt+2] = (IMX219_BG_Ratio_Typical&0xff00)>>8;
          IMX219OTPData[cnt+3] = (IMX219_BG_Ratio_Typical&0xff);
          IMX219OTPData[cnt+4] = (IMX219_GG_Ratio_Typical&0xff00)>>8;
          IMX219OTPData[cnt+5] = (IMX219_GG_Ratio_Typical&0xff);
      }
...
/****add by weiwenping****imx219 read lsc and awb****end****/
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("sensor detect success,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                break;
			}
            LOG_INF("Try %d read sensor id fail, write id: 0x%x, id: 0x%x\n", retry, imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		#ifdef CONFIG_YL_CAM_MODULE_NAME
		sprintf(cam_module_name, "imx219 unknow");
		#endif
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}

    printk("----IMX219_ReadLSC-AWB--start");
    IMX219_ReadLSC();

    if(0x08 == imx219_zhengqiao_module_id)
    {
        IMX219_RG_Ratio_Typical = 0x14b;
        IMX219_BG_Ratio_Typical = 0x121;
        IMX219_GG_Ratio_Typical = 0x203;

        //*sensor_id = imgsensor_info.sensor_id;
        sprintf(factory_module_id,"main_camera:8M-Camera imx219-ZhengQiao\n");
		#ifdef CONFIG_YL_CAM_MODULE_NAME
		sprintf(cam_module_name, "1 imx219 ZhengQiao");
		#endif
    }
    //else
    //{
    ...
    if(1 > info_limit)
    {
        ...
        get_camera_info(factory_module_id);
    }
    info_limit++;

	#ifdef CONFIG_YL_CAM_MODULE_NAME
	set_camera_module_name(cam_module_name);
	#endif
...
static kal_uint32 open(void)
{
    ...
    sensor_init();
    LOG_INF("remove LSC and use platform lsc check\n");
    //IMX219_ApplyLSC(tempData+16);
    iPreGain = 0;
