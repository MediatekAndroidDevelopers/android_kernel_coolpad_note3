...
#else
	#define LOG_INF(fmt, arg...)	do {} while(0)
#endif
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if(sensor_moudle_id==0x42)
    {

		RG_Ratio_Typical = 0x24f;
        BG_Ratio_Typical = 0x290;
        GG_Ratio_Typical = 0x400;
        LOG_INF("Read sensor_moudle_id == ar1335 oly \n");
        *sensor_id = AR1335_SENSOR_ID;
        sprintf(factory_module_id,"main_camera:13M-Camera ar1335-ofilm\n");     //display camera device info by miaolei@yulong.com 2015.05.26
		#ifdef CONFIG_YL_CAM_MODULE_NAME
		sprintf(cam_module_name, "ar1335 ofilm");
		#endif
    }
    else if(sensor_moudle_id==0x84)
    {
        RG_Ratio_Typical = 0x229;
        BG_Ratio_Typical = 0x241;
        GG_Ratio_Typical = 0x400;
        LOG_INF("Read sensor_moudle_id == ar1335 sunny\n");
        *sensor_id = AR1335_SENSOR_ID;
        sprintf(factory_module_id,"main_camera:13M-Camera ar1335-sunny\n");     //display camera device info by miaolei@yulong.com 2015.05.26
		#ifdef CONFIG_YL_CAM_MODULE_NAME
		sprintf(cam_module_name, "ar1335 sunny");
		#endif
    }
    else
    {
        *sensor_id = 0xffffffff;
		...
		#ifdef CONFIG_YL_CAM_MODULE_NAME
		sprintf(cam_module_name, "ar1335 unknown");
		#endif
    }

    if (*sensor_id != AR1335_SENSOR_ID) {
        *sensor_id = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
