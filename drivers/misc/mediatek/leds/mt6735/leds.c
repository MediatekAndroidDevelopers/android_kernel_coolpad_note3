...
static int Cust_Set_RGB_R(int level)
{
/*
		if (level)
			{
				gpio_set_value(GPIO_LED_G_MODE_PIN, 1);

			}
		else
			{
				gpio_set_value(GPIO_LED_B_MODE_PIN, 0);
			}
*/
		return 0;
...
//wangsl
static int Cust_Set_RGB_G(int level)
{
		if (level)
			{
				gpio_set_value(GPIO_LED_G_MODE_PIN, 1);
			...
			{
				gpio_set_value(GPIO_LED_G_MODE_PIN, 0);
			}
		return 0;
}
//wangsl
static int Cust_Set_RGB_B(int level)
{
		if (level)
			{
				gpio_set_value(GPIO_LED_B_MODE_PIN, 1);
			}
		else
			{
				gpio_set_value(GPIO_LED_B_MODE_PIN, 0);
		...
		for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
			...
			if (!led_node) {
				...
				case MT65XX_LED_MODE_GPIO:
					if (strcmp(pled_dtsi[i].name, "green") == 0) 
          					pled_dtsi[i].data =
          						(long)Cust_Set_RGB_G;
					...
					LEDS_DEBUG
					    ("kernel:[wsl]the backlight hw mode is GPIO.\n");
					break;
				#endif
