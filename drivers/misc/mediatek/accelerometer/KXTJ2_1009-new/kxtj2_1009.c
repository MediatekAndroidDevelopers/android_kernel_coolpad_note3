...
static int kxtj2_1009_init_client(struct i2c_client *client, int reset_cali)
{
	...
	{
printk("[lizh]kionix_1009 check devickID fail!\n");
		return res;
	}	
	...
	if(res != KXTJ2_1009_SUCCESS)
	{
printk("[lizh]kionix_1009 SetPowerMode fail!\n");
		return res;
	}
	...
	{
printk("[lizh]kionix_1009 SetBWRate fail!\n");
		return res;
	}
	...
	if(res != KXTJ2_1009_SUCCESS) //0x2C->BW=100Hz
	{
printk("[lizh]kionix_1009 SetDataFormat fail!\n");
		return res;
	}
	...
	{
printk("[lizh]kionix_1009 SetIntEnable fail!\n");
		return res;
	}
	...
	{ 
		...
		if(res != KXTJ2_1009_SUCCESS)
		{
printk("[lizh]kionix_1009 ResetCalibration fail!\n");
			return res;
		}
	}
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("[wsl] printk for konix-1004 addr is : 0x%x  \n",obj->hw->i2c_addr[0]);
	...
	#if defined(KOINIX_1009)
	new_client->addr = 0x0e;    //lizh
	#endif
	
	printk("[wsl] printk for start new_client = obj->client addr is : 0x%x  \n",obj->hw->i2c_addr[0]);
	
	printk("[wsl] printk for start new_client = obj->client addr is : 0x%x  \n",obj->client->addr);
	
	i2c_set_clientdata(new_client,obj);

	printk("[wsl] printk for end new_client = obj->client addr is : 0x%x  \n",obj->hw->i2c_addr[0]);
	...
	printk("[wsl] printk for  kxtj2_1009_i2c_client = new_client addr is : %d  \n",obj->hw->i2c_addr[0]);
	
	printk("[wsl] printk for  kxtj2_1009_i2c_client = new_client addr is : %x  \n",kxtj2_1009_i2c_client->addr);

	/* Init sensor */
	printk("[wsl] - ---kxtj2_1009_init_client ----start");
	
	if(0 != (err = kxtj2_1009_init_client(new_client, 1)))
	{
		printk("[wsl] - ---kxtj2_1009_init_client ----end");
