...
static int s4DW9718AF_ReadReg(unsigned short *a_pu2Result)
{
	...
	LOG_INF("[DW9718AF] s4DW9718AF_ReadReg \n");
	printk("[DW9718AF] s4DW9718AF_ReadReg \n");
	...
	printk("[DW9718AF] s4DW9718AF_ReadReg VCMMSB: %d \n",pBuff[1]);
	...
	printk("[DW9718AF] s4DW9718AF_ReadReg VCMLSB: %d \n",pBuff[0]);
	if (i4RetValue < 0) 
	{
...
static int s4AF_WriteReg(u16 a_u2Data)
{
	...
	printk("[DW9718AF] ENTER s4AF_WriteReg \n");
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		printk("[DW9817]s4DW9718AF_ReadReg pass!!! ++++++ \n ");
		if (ret == 0) {
			LOG_INF("Init Pos %6d\n", InitPos);
...
{
	printk("[DW9817]enter setAFMacro !!! ++++++ \n ");
	spin_lock(g_pAF_SpinLock);
...
{
	long i4RetValue = 0;
	printk("[DW9718AF_Ioctl] a_u4Command == %d \n",a_u4Command);
	switch (a_u4Command) {
...
{
	printk("[DW9817]enter DW9718AF_SetI2Cclient !!! pstAF_I2Cclient->addr == %x  ++++++ \n ",pstAF_I2Cclient->addr);
