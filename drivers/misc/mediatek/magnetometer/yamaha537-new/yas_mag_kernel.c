...
#include "yas.h"
#endif
#include <linux/sensors.h>/*add modify for chip info by qinxinjun@yulong.com in 20160504*/
...
#if YAS_MTK_DEBUG
#define MAGN_TAG        "[YAS537] "
#define MAGN_ERR(fmt, args...)  \
    printk(KERN_ERR  MAGN_TAG"yl_sensor_debug %s %d : "fmt, __func__, __LINE__, ##args)
...
struct mag_hw *yas_get_cust_mag(void)
{
	return &mag_cust;
}
...
/*static void input_get_data(struct input_dev *input, int *x, int *y,
        int *z, int *status)*///shihaobin modify
{
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (ret < 0)
    {
        MAGN_ERR("yl_sensor_debug %s show_position failed!\n",__func__);
        return -EFAULT;
    }
    return sprintf(buf, "%d\n", ret);
}
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (ret < 0)
    {
        MAGN_ERR("yl_sensor_debug %s store_position failed!\n",__func__);
        return -EFAULT;
    }
    return count;
}
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    //int xyz[3], status; shihaobin modify
    mutex_lock(&st->lock);
...
/************add 2015.5.25+++++++++++****************/
// modify start  by liuxinyuan@yulong.com  for reboot after enable magenable at 20150528
static ssize_t yas_magenable_show(struct device_driver *ddri, char *buf)
{
    struct yas_state *st = i2c_get_clientdata(this_client);
...
static ssize_t yas_magenable_store(struct device_driver *driver, const char *buf, size_t count)
{
    struct yas_state *st = i2c_get_clientdata(this_client);
    int enable;
    if (kstrtoint(buf, 10, &enable) < 0)
...
static ssize_t yas_magdata_show(struct device_driver *ddri, char *buf)

{
    ...
    //int xyz[3], status; shihaobin modify
    mutex_lock(&st->lock);
    //shihaobin@yulong.com modify for fastmmi test begin 20150611
    //input_get_data(st->cal, &xyz[0], &xyz[1], &xyz[2], &status);
    input_get_data(st->euler, &xyz[0], &xyz[1], &xyz[2], &status);
    //shihaobin@yulong.com modify for fastmmi test end 20150611
    mutex_unlock(&st->lock);
...
static DRIVER_ATTR(magenable,      S_IRUGO|S_IWUSR, yas_magenable_show, yas_magenable_store);
static DRIVER_ATTR(magsensordata,      S_IRUGO, yas_magdata_show, NULL);
// modify end by liuxinyuan@yulong.com  for reboot after enable magenable at 20150528
...
static void yas_work_func(struct work_struct *work)
{
    ...
    MAGN_ERR("yl_sensor_debug %s begin\n",__func__);
    ...
    MAGN_ERR("yl_sensor_debug %s end\n",__func__);
}
...
//static int yas_m_get_data(int *x, int *y, int *z, int *status)  shihaobin modify
static int yas_m_get_data(int32_t *x, int32_t *y, int32_t *z, int32_t *status)
{
    struct yas_state *st = i2c_get_clientdata(this_client);
...
/*shihaobin@yulong.com add for get device info 20150319 begin*/
extern int get_device_info(char* buf);
/*shihaobin@yulong.com add for get device info 20150319 end*/

extern uint8_t yas537_init_flag;  //shihaobin add for msensor auto detect 20150323

/*start to modify for chip info by qinxinjun@yulong.com in 20160504*/
static struct sensors_classdev sensors_magnetometer_cdev = {
         .name = "mag",
         .vendor = "yamaha537",
         .version = 1,
...
static int yas537_i2c_probe(struct i2c_client *i2c,
...
//    char tembuff[50]={0}; //add by shihaobin for device list ,20150319
    ...
    MAGN_ERR("yl_sensor_debug yas537_i2c_probe begin\n");
    ...
    /*start to modify for chip info by qinxinjun@yulong.com in 20160504*/
    sensors_classdev_register(&i2c->dev,&sensors_magnetometer_cdev);
    /*end to modify for chip info by qinxinjun@yulong.com in 20160504*/
    ...
    if (raw == NULL) {
        ret = -ENOMEM;
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe devide raw structure error\n");
        goto error_free_device;
    }
    cal = input_allocate_device();
    if (cal == NULL) {
        ret = -ENOMEM;
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe devide cal structure error\n");
        goto error_free_device;
    }
    euler = input_allocate_device();
    if (euler == NULL) {
        ret = -ENOMEM;
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe devide euler structure error\n");
        goto error_free_device;
    }
    ...
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe register raw device error\n");
        goto error_free_device;
    }
    ret = input_register_device(cal);
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe register cal device error\n");
        goto error_free_device;
    }
    ret = input_register_device(euler);
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe register euler device error\n");
        goto error_free_device;
    }
    ...
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe create cal sysfs group error\n");
        goto error_free_device;
    }
    ...
    if (ret)
    {
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe create euler sysfs group error\n");
        goto error_free_device;
    }

    atomic_set(&st->mag_enable, 0);
#ifdef YAS537_LEGACY	
    st->hw = yamaha537_get_cust_mag_hw();
#else
    st->hw = hw;
#endif
    st->raw = raw;
...
/*
    ...
    if (ret < 0) {
        ret = -EFAULT;
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe driver init error\n");
        goto error_remove_sysfs;
    }
    ret = st->mag.init();
    if (ret < 0) {
        ret = -EFAULT;
        MAGN_ERR("yl_sensor_debug yas537_i2c_probe st->mag.init error\n");
        goto error_remove_sysfs;
    }
    ret = yas537_create_attr(&(yas_init_info.platform_diver_addr->driver));
    if(ret < 0)
    {
        MAGN_ERR("yl_sensor_debug yas537_create_attr error! \n");
    }
    ...
    if (ret) {
        MAGN_ERR("yl_sensor_debug register mag control path ret\n");
        goto error_remove_sysfs;
    }
    ...
    if (ret) {
        MAGN_ERR("yl_sensor_debug register st control path ret\n");
        goto error_remove_sysfs;
    }
    ...
    /*shihaobin@yulong.com add for get device info 20150319 begin*/
   // sprintf(tembuff,"YAMAHA: yas537 \n");
    get_device_info(" MAGNETOMETER: yamaha537 \n"); // liuxinyuan@yulong.com add for get device info 20150521


    yas537_init_flag = 1; //shihaobin add for msensor auto detect 20150323
...
static int yas537_i2c_remove(struct i2c_client *i2c)
{
...
static const struct i2c_device_id yas_i2c_id[] = {
    {YAS_MTK_NAME, 0},
    { }
};
MODULE_DEVICE_TABLE(i2c, yas_i2c_id);
...
static struct i2c_driver yas537_i2c_driver = {
    .driver = {
        .name   = YAS_MTK_NAME,
        //.owner  = THIS_MODULE,
        //.pm = YAS_PM_OPS,
    ...
    .probe      = yas537_i2c_probe,
    .remove     = yas537_i2c_remove,
    .id_table   = yas_i2c_id,
};
...
{
#ifdef YAS537_LEGACY
    struct mag_hw *hw = yamaha537_get_cust_mag_hw();
    MAGN_ERR("yl_sensor_debug yas_local_init begin\n");
    MAGN_LOG("[%s]\n", __func__);
    yas_power(hw, 1);
#endif
    if (i2c_add_driver(&yas537_i2c_driver)) {
        MAGN_ERR("i2c_add_driver error\n");
        return -1;
    }

    //shihaobin add for msensor auto_detect 20150323 begin
    if(-1 == yas537_init_flag)
    {
        MAGN_ERR("device id not match");
        return -1;
    }
    //shihaobin add for msensor auto_detect 20150323 end

    MAGN_ERR("yl_sensor_debug yas_local_init end\n");
    return 0;
}
...
static int yas_local_uninit(void)
{
    ...
    yas_power(hw, 0);
#endif	
    i2c_del_driver(&yas537_i2c_driver);
    return 0;
}
