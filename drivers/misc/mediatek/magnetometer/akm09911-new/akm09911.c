...
static int mEnabled;
extern uint8_t akm09911_init_flag;
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	char strbuf[AKM09911_BUFSIZE];
    //shihaobin@yulong.com modify for FASTMMI test begin 20150604
    s16 data[3];
    ...
    data[0] = (s16)(sensor_data[13] * CONVERT_O / CONVERT_O_DIV);
    data[1] = (s16)(sensor_data[14] * CONVERT_O / CONVERT_O_DIV);
    data[2] = (s16)(sensor_data[15] * CONVERT_O / CONVERT_O_DIV);
    //shihaobin@yulong.com modify for fastmmi test begin 20150611
	...
	//shihaobin@yulong.com modify for FASTMMI test end 20150604
    return sprintf(buf, "%s\n", strbuf);
}
/*----------------------------------------------------------------------------*/
...
//shihaobin@yulong.com modify for FASTMMI test begin 20150604
static ssize_t show_sensorenable_value(struct device_driver *ddri, char *buf)
{
...
static ssize_t store_sensorenable_value(struct device_driver *driver, const char *buf, size_t count)
{
    int enable;
...
static DRIVER_ATTR(magenable,      S_IRUGO|S_IWUSR, show_sensorenable_value, store_sensorenable_value);
static DRIVER_ATTR(magsensordata,  S_IRUGO, show_sensordata_value, NULL);
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    struct i2c_msg company_msg[2], device_msg[2];
    ...
    mt_set_gpio_mode(GPIO_COMPASS_RST_PIN, GPIO_MODE_00);

    mt_set_gpio_dir(GPIO_COMPASS_RST_PIN, GPIO_DIR_OUT);

    mt_set_gpio_out(GPIO_COMPASS_RST_PIN,GPIO_OUT_ONE);

    msleep(5);

    mt_set_gpio_out(GPIO_COMPASS_RST_PIN, GPIO_OUT_ZERO);
