...
#define DRT_DRV_HALL_SUPPORT

#if defined(DRT_DRV_HALL_SUPPORT)

#define KPD_HALLKEY_OPEN  KEY_POWER
#define KPD_HALLKEY_CLOSE KEY_POWER

static void kpd_hallkey_handler(unsigned long data);
static DECLARE_TASKLET(kpd_hallkey_tasklet, kpd_hallkey_handler, 0);
extern void kpd_hallkey_handler_hal(unsigned long data);
...
static void kpd_hallkey_handler(unsigned long data)
{
    printk("[wsl] start kpd_hallkey_handler \n");
    kpd_hallkey_handler_hal(data);
}

irqreturn_t kpd_hallkey_eint_handler(int irq, void *dev_id)

{
//	printk("[wsl] start kpd_hallkey_eint_handler \n");
	disable_irq_nosync(hal_irq);
...
//zhuyifeng fixme
#if defined(DRT_DRV_HALL_SUPPORT)
    __set_bit(KPD_HALLKEY_OPEN, kpd_input_dev->keybit);
    __set_bit(KPD_HALLKEY_CLOSE, kpd_input_dev->keybit);
#endif
