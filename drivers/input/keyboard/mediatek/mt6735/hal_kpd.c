...
#define DRT_DRV_HALL_SUPPORT //wangsl
...
#if defined(DRT_DRV_HALL_SUPPORT)

static u8 kpd_hallkey_state = !KPD_HALLKEY_POLARITY;

int hal_irq;

void kpd_hallkey_handler_hal(unsigned long data)
{
	...
	u8 old_state;
	printk("[wsl] enter kpd_hallkey_handler_hal \n");
	
	kpd_hallkey_state = !kpd_hallkey_state;

	hall_keycode = ((kpd_hallkey_state == !!KPD_HALLKEY_POLARITY) ? KPD_HALLKEY_OPEN : KPD_HALLKEY_CLOSE);
	
 	/* for detecting the return to old_state */
	if(kpd_hallkey_state){

		input_report_key(kpd_input_dev, hall_keycode, 1);
		input_sync(kpd_input_dev);
		input_report_key(kpd_input_dev, hall_keycode, 0);
		input_sync(kpd_input_dev);
		old_state=IRQ_TYPE_LEVEL_LOW;
		}
		...
		input_report_key(kpd_input_dev, hall_keycode, 0);
		input_sync(kpd_input_dev);
		old_state=IRQ_TYPE_LEVEL_HIGH;
	}
	printk("[wsl] old_state = %d \n" ,old_state);
	irq_set_irq_type(hal_irq, old_state);	
	printk("[wsl] start disable_irq \n");
	enable_irq(hal_irq);
	printk("[wsl] end disable_irq \n");
}
...
//wangsl
#if defined(DRT_DRV_HALL_SUPPORT)
	extern irqreturn_t kpd_hallkey_eint_handler(int irq, void *dev_id);
	int ret;
	...
	printk("[wsl] mt_eint_register start for mahll \n");
	node = of_find_matching_node(node, mhall_of_match);
	
	if (node) {
		of_property_read_u32_array(node, "debounce", ints, ARRAY_SIZE(ints));
		gpio_set_debounce(ints[0], ints[1]);

		hal_irq = irq_of_parse_and_map(node, 0);
		
		if (1) {/*EINTF_TRIGGER*/
			ret =
			    request_irq(hal_irq, kpd_hallkey_eint_handler, IRQF_TRIGGER_HIGH,
					"mhall-eint", NULL);
			if (ret > 0) {
				ret = -1;
				printk("mhall request_irq IRQ LINE NOT AVAILABLE!.");
			}
		}
	 } else {
		printk("kpd request_irq can not find kpd eint device node!.");
		ret = -1;
	}

//	gpio_request(KPD_HALLKEY_EINT, "hallkey");                      
//	gpio_direction_input(KPD_HALLKEY_EINT);
//	hal_irq = gpio_to_irq(KPD_HALLKEY_EINT);
//	printk("[wsl] gpio_to_irq success \n");
//	gpio_set_debounce(KPD_HALLKEY_EINT, KPD_HALLKEY_DEBOUNCE);
//	printk("[wsl]gpio set debounce success \n");
//	ret=request_irq(hal_irq, kpd_hallkey_eint_handler,IRQF_TRIGGER_HIGH, "MHALL-eint", NULL);
//	if (ret > 0) {
//
