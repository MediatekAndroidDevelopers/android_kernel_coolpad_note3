...
#define TW_BOEN_ID_B     0
#define TW_YEJI_ID_B     1
#define TW_SHENYUE_ID_B  2
#define TW_BOEN_ID       3
#define TW_YEJI_ID       4
#define TW_SHENYUE_ID    5
...
int goodix_get_firmware_version( char *buf)
{
    ...
    tp_type = goodix_read_sensor_id(goodix_ts);//read tp type: OFILM or BOEN
    GTP_DEBUG(" 0x%x::%d\n",version_info,tp_type);

    if(tp_type == TW_BOEN_ID_B) {
        GTP_DEBUG("TW_BOEN_ID_B::0x%x::%d\n",version_info,tp_type);
        ret = sprintf(buf, "BOEN:GT9157:0x%04x:%2x\n",version_info,cfg_info_group1[0]);
        GTP_DEBUG("TW_BOEN_ID::%s\n",buf);
        return ret;
    }

    if(tp_type == TW_YEJI_ID_B) {
        GTP_DEBUG("TW_YEJI_ID_B::0x%x::%d\n",version_info,tp_type);
        ret = sprintf(buf, "YEJI:GT9157:0x%04x:%2x\n",version_info,cfg_info_group2[0]);
        GTP_DEBUG("TW_YEJI_ID::%s\n",buf);
        return ret;
    }

    if(tp_type == TW_SHENYUE_ID_B) {
        GTP_DEBUG("TW_SHENYUE_ID_B::0x%x::%d\n",version_info,tp_type);
        ret = sprintf(buf, "SHENYUE:GT9157:0x%04x:%2x\n",version_info,cfg_info_group3[0]);
        GTP_DEBUG("TW_SHENYUE_ID::%s\n",buf);
        return ret;
    }

    if(tp_type == TW_BOEN_ID) {
        GTP_DEBUG("TW_BOEN_ID::0x%x::%d\n",version_info,tp_type);
        ret = sprintf(buf, "BOEN:GT9157:0x%04x:%2x\n",version_info,cfg_info_group4[0]);
        GTP_DEBUG("TW_BOEN_ID::%s\n",buf);
        return ret;
    }

    if(tp_type == TW_YEJI_ID) {
        GTP_DEBUG("TW_YEJI_ID::0x%x::%d\n",version_info,tp_type);
        ret = sprintf(buf, "YEJI:GT9157:0x%04x:%2x\n",version_info,cfg_info_group5[0]);
        GTP_DEBUG("TW_YEJI_ID::%s\n",buf);
        return ret;
    }

    if(tp_type == TW_SHENYUE_ID) {
        GTP_DEBUG("TW_SHENYUE_ID::0x%x::%d\n",version_info,tp_type);
        ret = sprintf(buf, "SHENYUE:GT9157:0x%04x:%2x\n",version_info,cfg_info_group6[0]);
        GTP_DEBUG("TW_SHENYUE_ID::%s\n",buf);
        return ret;
    }
...
/*end qinxinjun@yulong.com 20160227 modify module ID for factory test*/
...
//start: modified by zhoumaiyun@yulong.com
int goodix_vendor(char * vendor)
{
#if 0
    unsigned char tp_type = -1;
    char* moduler = "";
    tp_type = goodix_read_sensor_id(goodix_ts);
    if(tp_type == TW_BOEN_ID_B)
        moduler = "BoEn_b";
    else if(tp_type == TW_YEJI_ID_B)
        moduler = "YeJi_b";
    else if(tp_type == TW_SHENYUE_ID_B)
        moduler = "ShenYue_b";
    else if(tp_type == TW_BOEN_ID)
        moduler = "BoEn";
    else if(tp_type == TW_YEJI_ID)
        moduler = "YeJi";
    else if(tp_type == TW_SHENYUE_ID)
        moduler = "ShenYue";
    else
        moduler = "Unknown";

    sprintf(vendor,"%s:%s\n","goodix:GT9157",moduler);
#else
    sprintf(vendor,"%s\n","goodix:GT9157");
#endif
...
//end: modified by zhoumaiyun@yulong.com
...
/**** modified by phg start for gesture wake up *********/
    ...
    //              }
                ...
                if (DOZE_WAKEUP == doze_status)
                {
                        ...
                        doze_buf[2] = 0x00;
                        gtp_i2c_write(i2c_client_point, doze_buf, 3);
                        gtp_enter_doze(i2c_client_point); //modified by zhoumaiyun@yulong.com 2016.05.04
                }
            }
            mutex_unlock(&i2c_access);
            continue;
...
#ifdef TPD_POWER_SOURCE_CUSTOM
    //modify to poweron when system suspend by qinxinjun@yulong.com in 20160512
