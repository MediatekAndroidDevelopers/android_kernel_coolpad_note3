...
//wangsl
#if defined(CONFIG_DRT_TPD_PROXIMITY)
...
static unsigned int TPD_RES_Y1 = 960;
static unsigned int  TPD_RES_X1 = 540;
...
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	...
	else if((writelen > 8)&&(0 != g_dma_buff_pa))
	{
		...
		client->addr = (client->addr & I2C_MASK_FLAG )| I2C_DMA_FLAG;
	    //client->ext_flag = client->ext_flag | I2C_DMA_FLAG;
	    ret = i2c_master_send(client, (unsigned char *)g_dma_buff_va, writelen);
	    client->addr = client->addr & I2C_MASK_FLAG & (~I2C_DMA_FLAG);
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (tpd_touchinfo(&cinfo, &pinfo)) {
			...
			if ((cinfo.y[0] >= TPD_RES_Y1) && (pinfo.y[0] < TPD_RES_Y1)
			...
			if (tpd_dts_data.use_tpd_button) {
				if ((cinfo.y[0] <= TPD_RES_Y1 && cinfo.y[0] != 0) && (pinfo.y[0] > TPD_RES_Y1)
				&& ((pinfo.p[0] == 0) || (pinfo.p[0] == 2))) {
...
reset_proc:
	...
	//zhuyifeng, report fw version
	{
		...
		fts_read_reg(client, FTS_REG_FW_VER, &fw_version);
		
		sprintf(info, "FT6336 V%d", fw_version);
...
#endif
*/
	TPD_RES_Y1 = DISP_GetScreenHeight();
	TPD_RES_X1 = DISP_GetScreenWidth();
	#ifdef SYSFS_DEBUG
...
static void tpd_resume(struct device *h)
{
#if defined(TPD_PROXIMITY) //wangsl
	if (tpd_proximity_flag == 1)
	{		
