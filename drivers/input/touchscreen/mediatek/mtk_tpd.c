...
/*static */void TP_SetTP_Name_info(char *tp_name); //wangkaifeng
		...
		if (tpd_driver_list[i].tpd_device_name != NULL) {
			...
			if (tpd_load_status == 1) {
				TPD_DMESG("[mtk-tpd]tpd_probe, tpd_driver_name=%s\n",
					  tpd_driver_list[i].tpd_device_name);
				g_tpd_drv = &tpd_driver_list[i];
				//TP_SetTP_Name_info(tpd_driver_list[i].tpd_device_name); //wangkaifeng
				break;
			}
		}
	}
...
/*static */void TP_SetTP_Name_info(char *tp_name)
{
	if (NULL == tp_name)
	{
		//printk("<<<< alex.shi %s(): TP_SetTP_Name_info null!\n", __func__);
		return;
	}
