...
static int found_full_block_to_archive(struct ubi_device *ubi, struct rb_root *root)
{
	struct rb_node *p;
	struct ubi_wl_entry *e = NULL;
	int i=0, j, ret;
	...
	for(i=0;i<MAX_CHECK_PEBS;i++) {
		...
		for (j = 0; j < ubi->min_io_size; j++) {
			if (((const uint8_t *)buf)[j] != 0xFF) {
				dbg_wl("peb %d is full %d", peb, ((const uint8_t *)buf)[j]);
				full = 1;
				break;
			}
		}
...
static int ensure_wear_leveling(struct ubi_device *ubi, int nested)
{
	...
	if (ubi->attaching == 0 && ubi->free_count <= ((ubi->peb_count - ubi->mtbl_slots) / SLC_THRESHOLD))
		found_full_block_to_archive(ubi, &ubi->used);
#endif
